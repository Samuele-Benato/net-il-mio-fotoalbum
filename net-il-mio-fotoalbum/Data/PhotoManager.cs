...
namespace net_il_mio_fotoalbum.Data
{
    public class PhotoManager
    {
        ...
        public static Photo GetPhoto(int id, string userId, bool includereferences = true)
        {
            using (PhotoContext db = new PhotoContext())
                if (includereferences)
                    return db.Photos.Where(r => r.Id == id && r.UserId == userId).Include(c => c.Categories).FirstOrDefault();

            using (PhotoContext db = new PhotoContext())
                return db.Photos.FirstOrDefault(p => p.Id == id && p.UserId == userId);
        }
        ...
        public static bool EditPhoto(int id, string userId, string title, string description, byte[] imageFile, bool visible, List<string> categories)
        {
            using PhotoContext db = new PhotoContext();
            var photo = db.Photos.Where(x => x.Id == id && x.UserId == userId).Include(x => x.Categories).FirstOrDefault();
        ...
        public static bool DeletePhoto(int id, string userId)
        {
            using PhotoContext db = new PhotoContext();
            var photo = db.Photos.FirstOrDefault(p => p.Id == id && p.UserId == userId);
        ...
        public static void Seed()
        {
            ...
            if(CountPhotos() == 0)
            {
                InsertPhoto(new Photo("Il Primo Sguardo", "Un momento intimo catturato durante il matrimonio di Anna e Luca, dove gli sposi si vedono per la prima volta. La luce naturale che filtra dalle finestre crea un'atmosfera magica, evidenziando l'emozione nei loro occhi.", true, "e7fccd5e-f363-453a-8a06-446ff70febaf"));
                InsertPhoto(new Photo("Ombre Metropolitane", "Una foto in bianco e nero che ritrae una strada di una grande città al tramonto. Le ombre lunghe e le silhouette dei passanti creano un gioco di contrasti che esalta l'architettura urbana e l'atmosfera frenetica della vita cittadina.", true, "e7fccd5e-f363-453a-8a06-446ff70febaf"));
                InsertPhoto(new Photo("L'Alba in Montagna", "Un'immagine mozzafiato delle montagne al sorgere del sole. I colori pastello del cielo si riflettono sulla neve immacolata, creando un panorama sereno e pacifico. Un capriolo solitario si staglia all'orizzonte, aggiungendo un tocco di vita selvaggia.", true, "e7fccd5e-f363-453a-8a06-446ff70febaf"));
                InsertPhoto(new Photo("Momenti di Vita Quotidiana", "Una fotografia che cattura l'essenza della vita di un piccolo quartiere di città. Un anziano signore siede su una panchina, osservando i bambini che giocano mentre un venditore ambulante prepara il suo carretto. L'immagine racconta storie di quotidianità e tradizione.", true, "e7fccd5e-f363-453a-8a06-446ff70febaf"));
                InsertPhoto(new Photo("Riflessi dell'Anima", "Un ritratto intenso di una giovane donna con un'espressione profonda e pensierosa. Gli occhi sono il punto focale della foto, riflettendo luce e ombre che sembrano svelare emozioni nascoste. Lo sfondo sfocato mette in risalto la figura e aggiunge un senso di mistero.", true, "e7fccd5e-f363-453a-8a06-446ff70febaf"));
            }
